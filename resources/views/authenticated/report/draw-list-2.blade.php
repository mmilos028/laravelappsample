        ...
        <section class="content">
            <div class="box">
                ...
                <div class="box-header" id="filterContainer">
                    <div class="row">
                        <div class="col-md-2" @php if($logged_in_subject_dtype == config("constants.COLLECTOR_TYPE_NAME")) {  echo "style=\"display:none;\""; } @endphp>
                            <label class="control-label">{{__('authenticated.Draw Model')}}</label>
                            <select class="form-control" id="drawModels" name="drawModels">
                                <option value="">{{trans("authenticated.All")}}</option>
                            </select>
                        </div>
                        <div class="col-md-2">
                            <label class="control-label">{{__('authenticated.Draw Status')}}</label>
                            <select class="form-control" id="draw_status" name="draw_status">{{ __("authenticated.SCHEDULED") }}
                                <option value="">{{trans("authenticated.All")}}</option>
                                <option value="-1">{{ __("authenticated.SCHEDULED") }}</option>
                                <option value="0">{{ __("authenticated.IN PROGRESS") }}</option>
                                <option value="1">{{ __("authenticated.COMPLETED") }}</option>
                            </select>
                        </div>
                        <div class="col-md-2">
                            <label class="control-label">{{__('authenticated.Currency')}}</label>
                            <select class="form-control" id="currency" name="currency">
                                <option value="">{{trans("authenticated.All")}}</option>
                            </select>
                        </div>
                        <div class="col-md-2" style="display:none;">
                            <label class="control-label">{{__('authenticated.Affiliate1')}}</label>
                    ...
                    <div class="row">
                        <div class="col-md-2">
                            <label class="control-label">{{__('authenticated.Draw ID')}}</label>
                            <input type="text" id="drawID" class="form-control" placeholder="{{__('authenticated.Draw ID')}}">
                        </div>
                        <div class="col-md-2">
                            <label class="control-label">{{__('authenticated.Draw SN')}}</label>
                            <input type="text" id="drawSN" class="form-control" placeholder="{{__('authenticated.Draw SN')}}">
                        </div>
                    ...
                    <div id="tableContainer">
                        <table style="width: 100%; font-size: 11px !important;" id="draw-list-report" class="table table-bordered table-hover table-striped pull-left">
                            ...
                            <tr class="bg-blue-active">
                                ...
                                <th style="text-align: left;">{{ __("authenticated.Draw SN") }}</th>
                                <th style="text-align: left;">{{ __("authenticated.Draw Model") }}</th>
                                <th style="text-align: left;">{{ __("authenticated.Super Draw Coefficient") }}</th>
                                <th style="text-align: left;">{{ __("authenticated.Date & Time") }}</th>
                                <th style="text-align: left;">{{ __("authenticated.Tickets for Draw") }}</th>
                                <th style="text-align: left;">{{ __("authenticated.Total Bet for Draw") }}</th>
                                <th style="text-align: left;">{{ __("authenticated.Win for Draw") }}</th>
                                <th style="text-align: left;">{{ __("authenticated.Currency") }}</th>
                                <th style="text-align: left;">{{ __("authenticated.Status") }}</th>
                                <th style="text-align: left;">{{ __("authenticated.Local JP Amount") }}</th>
                                <th style="text-align: left;">{{ __("authenticated.Global JP Amount") }}</th>
                                <th style="text-align: left;">{{ __("authenticated.Star Double Up") }}</th>
                            </tr>
                            </thead>
                        </table>
                    </div>
    ...
    <script>
        $(document).ready(function(){
            ...
            function getReportData(){
                ...
                $.ajax({
                    method: "GET",
                    url: "{{ LaravelLocalization::getLocalizedURL(LaravelLocalization::getCurrentLocale(), "/draw-list-report") }}",
                    "dataSrc": "result",
                    data: {
            ...
            function listDrawModels(){
                $.ajax({
                    method: "GET",
                    url: "{{ LaravelLocalization::getLocalizedURL(LaravelLocalization::getCurrentLocale(), "listAllDrawModels") }}",
                    global: false,
                    dataType: "json",
                    //data: "result",
                    //"dataSrc": "result",
