...
@section('content')
    ...
    <div class="content-wrapper">
        <section class="content-header">
            <h1><i class="fa fa-bar-chart"></i>{{ __("authenticated.Collector Transactions") }}&nbsp;<button id="showContextualMessage" class="btn btn-primary"><strong class="fa fa-question-circle"></strong></button></h1>
            <ol class="breadcrumb">
                <li><i class="fa fa-bar-chart"></i> {{ __("authenticated.Reports") }}</li>
                <li class="active">{{ __("authenticated.Collector Transactions") }}</li>
            </ol>
        </section>
                                ...
                                <div class="row" id="fullReportContainer">
                                    <table id="fullReport" class="table table-bordered dataTable pull-left" style="width: 100%; font-size: 13px !important;">
                                        ...
                                        <tr id="tableHeader" class="bg-blue-active headerFontXL">
                                            <th style="text-align: left;">{{__ ("authenticated.No. Of Transactions")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Total Collected")}}</th>
                                ...
                                <div class="row" id="transactionReportContainer">
                                    <table id="transactionReport" class="table table-bordered dataTable pull-left" style="width: 100%; font-size: 13px !important;">
                                        ...
                                        <tr id="tableHeader2" class="bg-blue-active labelXS">
                                            <th style="text-align: left;">{{__ ("authenticated.Datum & Time")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Transaction ID")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.From User")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.From Entity")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.To User")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.To Entity")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Amount")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Currency")}}</th>
                                        </tr>
                                        </thead>
                                    </table>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body bg-info">
                    <p id="modal-body-p">
                        <strong>{{trans("authenticated.This report shows collector transactions.")}}</strong>
                        <br><br>
    ...
    <script>
        $(document).ready(function(){
            ...
            var tree = $('#tree1').treegrid({
                method: 'GET',
                url: '{{ LaravelLocalization::getLocalizedURL(LaravelLocalization::getCurrentLocale(), "/report/financial-report-get-subject-tree-users") }}?listType=' + "2",
                //data: "result",
            ...
            function getReportData(selected_subject_id, fromDate, toDate){
                $.ajax({
                    method: "GET",
                    url: "{{ LaravelLocalization::getLocalizedURL(LaravelLocalization::getCurrentLocale(), "collectorTransactionReportAjax") }}",
                    data:{
                    ...
                    columns:[
                        ...
                        },
                        {
                            data: function(row, type, val, meta){
                                ...
                                if(id == "{{config('constants.ROOT_MASTER_ID')}}" || id == "{{config('constants.LUCKY_SIX_ID')}}"){
