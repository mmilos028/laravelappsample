...
@section('content')
    ...
    <div class="content-wrapper">
        <section class="content-header">
            <h1><i class="fa fa-bar-chart"></i>{{ __("authenticated.All Transactions") }}&nbsp;
            ...
            <ol class="breadcrumb">
                ...
                <li class="active">{{ __("authenticated.All Transactions") }}</li>
            </ol>
        </section>
                        ...
                        <div class="col-md-8">
                            @if($agent->isDesktop())
                                <div class="row">
                                    ...
                                    <div class="col-md-1 padding">
                                        <button id="toggleTreeGrid" class="btn btn-block btn-sm btn-info" data-toggle="tooltip" data-placement="top" title="{{ __("authenticated.Toggle") }}" disabled>
                                            <span id="toggleSpan" class = "fa fa-toggle-on"></span>
                                        </button>
                                    </div>
                                    <div class="col-md-1 padding" style="display: none;">
                                        <button id="collapseToRootBtn" class="btn btn-block btn-sm btn-primary" data-toggle="tooltip" data-placement="top" title="{{ __("authenticated.Home") }}" onclick="collapseAll()">
                                            <span class = "fa fa-home"></span>
                                        </button>
                                    </div>
                                    <div class="col-md-1 padding">
                                        <button id="showDirectTreeBtn" class="btn btn-block btn-sm btn-primary" data-toggle="tooltip" data-placement="top" title="{{ __("authenticated.Direct") }}" onclick="collapseNotRoot()">
                                            <span class = "fa fa-home"></span>
                                        </button>
                                    </div>
                                    <div class="col-md-1 padding">
                                        <button id="expandTreeBtn" class="btn btn-block btn-sm btn-primary" data-toggle="tooltip" data-placement="top" title="{{ __("authenticated.All") }}" onclick="expandAll()">
                                ...
                                <div class="row" id="compressedReportContainer" style="display: none;">
                                    <table id="compressedReport" class="table table-bordered dataTable pull-left" style="width: 100%; font-size: 13px !important;">
                                        ...
                                        <tr id="tableHeader" class="bg-blue-active labelXS">
                                            <th style="text-align: left;">{{__ ("authenticated.No.Of Tickets")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Profit In")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Profit Out")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Profit Result")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Collected")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Withdraw")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Difference")}}</th>
                                ...
                                <!--<div class="row" style="display: none;" id="fullReportContainer">
                                    <table id="fullReport" class="table table-bordered dataTable pull-left" style="width: 100%; font-size: 13px !important;">
                                        ...
                                        <tr id="tableHeader" class="bg-blue-active headerFontXL">
                                            <th style="text-align: left;">{{__ ("authenticated.No. Of Tickets")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.No. Of Wins")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.No. Of Canceled Tickets")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Total Deposit")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Cashier Deposit")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Online Deposit")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Canceled Deposit")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Total Withdraw")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Cashier Withdraw")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Online Withdraw")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Netto")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Currency")}}</th>
                                        </thead>
                                    </table>
                                </div>-->
                                <div class="row">
                                    <table id="transactionReport" class="table table-bordered dataTable pull-left" style="width: 100%; font-size: 13px !important;">
                                        ...
                                        <tr id="tableHeader2" class="bg-blue-active labelXS">
                                            ...
                                            <th style="text-align: left;">{{__ ("authenticated.Transaction ID")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Transaction Type")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.From Start Balance")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Deposit")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Withdraw")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.From End Balance")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Currency")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.To Start Balance")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.To End Balance")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.To Subject")}}</th>
                                            <th style="text-align: left;">{{__ ("authenticated.Executed By")}}</th>
                                        </thead>
                                    </table>
                                </div>
                            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body bg-info">
                    <p id="modal-body-p">
                        <strong>{{trans("authenticated.This report shows profit (ticket) transactions, collector transactions as well as subject mutual deposit, withdraw transactions.")}}</strong>
                        <br><br>
                        {{trans("authenticated.Data is retrieved per subject, for selected period, and is displayed in 1 table.Table represents list of transactions for selected subject and period.")}}
                    </p>
                </div>
    ...
    <script>
        $(document).ready(function(){
            ...
            var tree = $('#tree1').treegrid({
                method: 'GET',
                url: '{{ LaravelLocalization::getLocalizedURL(LaravelLocalization::getCurrentLocale(), "/report/financial-report-get-subject-tree-users") }}?listType=' + "-1",
                //data: "result",
            ...
            function getReportData(selected_subject_id, fromDate, toDate){
                $.ajax({
                    method: "GET",
                    url: "{{ LaravelLocalization::getLocalizedURL(LaravelLocalization::getCurrentLocale(), "allTransactionsReportAjax") }}",
                    data:{
